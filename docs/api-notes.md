...
- **Installed-app OAuth2** (refresh token): Cloud projekt s povoleným Google Ads API + OAuth consent screen (External) + klient typu Desktop app. Consent screen dnes žije pod **Google Auth Platform** (`console.cloud.google.com/auth/` → Branding / Audience / Clients), ne pod „APIs & Services → OAuth consent screen" jako ve starších návodech. **Consent screen v režimu „Testing" = refresh token expiruje po 7 dnech** (Google Identity docs) → `invalid_grant` při refreshi. Řešení: publishing status „In production" (neověřená appka ukáže varování, ale funguje; ověření pro vlastní nástroj není nutné). Další důvody zneplatnění: odvolání přístupu, 6 měsíců nepoužití, >100 živých refresh tokenů na klienta, změna hesla (jen Gmail scopes).

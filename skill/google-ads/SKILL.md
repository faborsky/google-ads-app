...
**Running `auth` yourself:** it opens a browser and **blocks until the user finishes signing in** — Google account, the "unverified app" warning, and a passkey prompt easily take more than the default Bash timeout, and a killed process writes nothing while the browser still says success. Run it with `timeout: 600000` (10 min), tell the user beforehand that a browser will open and that you are waiting for them, and confirm the result with `accounts` rather than assuming. Never pass `--print` — it is the only path that puts the refresh token into your context; without it the token goes straight to `.env` and neither stdout nor `--json` ever contains it. `keywords-research` needs a developer token with **Basic Access** (the default Explorer Access blocks Keyword Planner) — if it fails with an authorization error, say so instead of retrying.
